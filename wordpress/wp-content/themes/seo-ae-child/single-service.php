<?php
/**
 * Single Service Template — Enterprise Redesign
 * SearchEngineOptimization.ae (child theme)
 *
 * Reuses the homepage redesign design system (.rhome / .rh-*) plus service-specific
 * styles. Assets enqueued in functions.php on is_singular('service').
 */

/* FAQPage JSON-LD — registered before get_header() fires wp_head(). */
...
$sid          = get_the_ID();
$title        = get_post_field( 'post_title', $sid ); // raw title (bypasses the_title blanking on Elementor pages)
$title_short  = trim( preg_replace( '/\s*(?:Services?|Company|Agency)\b.*$/i', '', $title ) ) ?: $title; // short label for prose/CTAs
...
/* Icon by category/title keyword */
$slug = get_post_field( 'post_name', $sid );
$icon = 'search';
$map  = [ 'ai' => 'ai', 'ppc' => 'ppc', 'ads' => 'ppc', 'social' => 'social', 'design' => 'design', 'develop' => 'code', 'web-dev' => 'code', 'content' => 'report', 'link' => 'link', 'local' => 'pin' ];
foreach ( $map as $k => $v ) { if ( strpos( $slug, $k ) !== false ) { $icon = $v; break; } }

/* Benefit strings (support ACF repeater or plain strings) */
$benefit_list = [];
foreach ( (array) $benefits as $b ) { $benefit_list[] = is_array( $b ) ? ( $b['benefit'] ?? '' ) : $b; }
$benefit_list = array_values( array_filter( $benefit_list ) );
if ( ! $benefit_list ) {
	$benefit_list = [ 'Dedicated senior SEO specialists', 'Technical, content &amp; authority coverage', 'Transparent weekly reporting', 'Bilingual (Arabic + English) strategy', 'ROI &amp; revenue focused execution' ];
}

/* Content with heading IDs + TOC.
   Elementor-built pages: render the stored plain content (wpautop) instead of the
   Elementor builder layout, which is designed for full-width and breaks in a column. */
...
$toc = [];
$content_html = preg_replace_callback( '/<h2([^>]*)>(.*?)<\/h2>/is', function ( $m ) use ( &$toc ) {
	$text = trim( wp_strip_all_tags( $m[2] ) );
	if ( $text === '' ) return $m[0];
	$id = sanitize_title( $text ); $base = $id; $n = 2;
	while ( isset( $toc[ $id ] ) ) { $id = $base . '-' . $n++; }
	$toc[ $id ] = $text;
	$attrs = $m[1];
	if ( stripos( $attrs, 'id=' ) === false ) $attrs .= ' id="' . esc_attr( $id ) . '"';
...
<main class="rhome rservice">
  ...
  <section class="rh-hero rs-hero">
    ...
    <div class="rh-wrap">
      <div class="rs-hero__grid">
        <div data-reveal>
          <div class="rs-crumbs"><a href="<?php echo esc_url( home_url('/') ); ?>">Home</a> &rsaquo; <a href="<?php echo esc_url( home_url('/services/') ); ?>">Services</a> &rsaquo; <?php echo esc_html( $title ); ?></div>
          ...
          <div class="rs-byline">
            ...
            <div><div class="rs-byline__nm"><?php echo esc_html( $author_name ); ?></div><div class="rs-byline__mt"><?php echo esc_html( $author_role ); ?> &middot; <?php echo esc_html( $reading_time ); ?> min read</div></div>
          </div>
          <div class="rh-hero__cta">
            <a href="<?php echo esc_url( home_url('/contact/') ); ?>" class="rh-btn rh-btn--primary"><?php echo rh_icon('rocket',18); ?> <?php echo esc_html( $cta_primary ); ?></a>
            <a href="mailto:<?php echo esc_attr( $email ); ?>" class="rh-btn rh-btn--ghost"><?php echo rh_icon('mail',18); ?> Email Us</a>
          </div>
          <div class="rs-hero__badges">
            <div class="rs-badge"><span><?php echo rh_icon('shield',18); ?></span> 100% White-Hat SEO</div>
            <div class="rs-badge"><span><?php echo rh_icon('chart',18); ?></span> Data-Driven Approach</div>
            <div class="rs-badge"><span><?php echo rh_icon('target',18); ?></span> ROI-Focused Strategy</div>
          </div>
        </div>

        <div class="rs-hero__card" data-reveal data-reveal-delay="2">
          <div class="rs-hero__card-top">
            <span class="rs-hero__card-ic"><?php echo rh_icon( $icon, 26 ); ?></span>
            <div><div class="rs-hero__card-nm"><?php echo esc_html( $hero_badge ); ?></div><div class="rs-hero__card-sb"><?php echo esc_html( $hero_sub ); ?></div></div>
          </div>
          <ul>
            <?php foreach ( array_slice( $benefit_list, 0, 6 ) as $b ) : ?>
  ...
  <!-- ===================== OVERVIEW / CONTENT ===================== -->
  <?php if ( trim( wp_strip_all_tags( $content_raw ) ) ) : ?>
      ...
      <div class="rs-body">
        <div class="rs-content">
          ...
          <nav class="rs-toc" aria-label="On this page">
            ...
            <ol><?php foreach ( $toc as $id => $label ) : ?><li><a href="#<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $label ); ?></a></li><?php endforeach; ?></ol>
          </nav>
          <?php endif; ?>
        ...
        <aside class="rs-aside">
          <div class="rs-getcard" data-reveal data-reveal-delay="1">
            <h3>What you get</h3>
            <ul><?php foreach ( array_slice( $benefit_list, 0, 7 ) as $b ) : ?><li><?php echo rh_icon('check',18); ?><span><?php echo wp_kses_post( $b ); ?></span></li><?php endforeach; ?></ul>
          </div>
          <div class="rs-cta-card" data-reveal data-reveal-delay="2">
            ...
            <p>See exactly where your biggest <?php echo esc_html( strtolower( $title_short ) ); ?> opportunities are.</p>
            <a href="<?php echo esc_url( home_url('/contact/') ); ?>" class="rh-btn rh-btn--primary" style="width:100%"><?php echo rh_icon('rocket',18); ?> Get Free Audit</a>
          </div>
        </aside>
      </div>
    </div>
  ...
  $steps = [];
  if ( $process ) {
    $ic = [ 'search','gauge','target','bolt','chart','trend' ]; $n = 0;
    foreach ( array_slice( (array) $process, 0, 6 ) as $st ) {
      $steps[] = [ $st['step_title'] ?? '', $st['step_desc'] ?? '', $ic[ $n % 6 ] ]; $n++;
    }
  }
  if ( count( $steps ) < 3 ) {
    $steps = [
  ...
  <section class="rh-section rh-dark">
    <div class="rh-wrap">
      <div class="rh-head-center" data-reveal>
        ...
        <p class="rh-lead">A transparent, data-driven system engineered to turn <?php echo esc_html( strtolower( $title_short ) ); ?> into measurable growth.</p>
      </div>
      <div class="rs-steps" data-reveal style="grid-template-columns:repeat(<?php echo count( $steps ); ?>,1fr)">
        <?php foreach ( $steps as $n => $st ) : ?>
          <div class="rs-step">
            <div class="rs-step__n"><?php echo sprintf('%02d', $n + 1); ?></div>
  ...
  <section class="rh-section rh-section--tight rh-gray">
    <div class="rh-wrap">
      <div class="rh-head-center" data-reveal><span class="rh-eyebrow"><?php echo rh_icon('code',14); ?> Tools we use</span><h2 class="rh-h2">Enterprise-grade tooling</h2></div>
      <div class="rs-tech" data-reveal>
        <?php foreach ( $technologies as $t ) : ?><span><?php echo esc_html( is_array( $t ) ? ( $t['tech_name'] ?? '' ) : $t ); ?></span><?php endforeach; ?>
      </div>
    </div>
  </section>
  <?php endif; ?>
        ...
        <?php foreach ( $cases as $ci => $c ) : ?>
          <article class="rh-case" data-reveal data-reveal-delay="<?php echo esc_attr( $ci ); ?>">
            <div class="rh-case__head">
              <div class="rh-case__client"><div class="rh-case__logo"><?php echo esc_html( $c[2] ); ?></div><div><div class="rh-case__name"><?php echo esc_html( $c[0] ); ?></div><div class="rh-case__sector"><?php echo esc_html( $c[1] ); ?></div></div></div>
            ...
            <div class="rh-case__chart">
              <svg viewBox="0 0 320 130" preserveAspectRatio="none">
                <defs><linearGradient id="rsCase<?php echo $ci; ?>" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="rgba(28,200,255,.30)"/><stop offset="1" stop-color="rgba(28,200,255,0)"/></linearGradient></defs>
                <path d="<?php echo esc_attr( $c[7] ); ?> L320,130 L0,130 Z" fill="url(#rsCase<?php echo $ci; ?>)"/>
            ...
            <div class="rh-case__foot"><a class="rh-btn rh-btn--outline" href="<?php echo esc_url( home_url('/case-studies/') ); ?>">View Case Study <?php echo rh_icon('arrow',16); ?></a></div>
          </article>
        <?php endforeach; ?>
      </div>
    </div>
  ...
  $others = $related;
  if ( ! $others ) {
    $others = get_posts( [ 'post_type' => 'service', 'posts_per_page' => 3, 'post__not_in' => [ $sid ], 'orderby' => 'rand' ] );
  }
  if ( $others ) : ?>
  <section class="rh-section rh-gray">
    <div class="rh-wrap">
      <div class="rh-head-center" data-reveal><span class="rh-eyebrow"><?php echo rh_icon('bolt',14); ?> Explore more</span><h2 class="rh-h2">Related services</h2></div>
      <div class="rh-serv">
        <?php $oi = 0; foreach ( array_slice( $others, 0, 4 ) as $svc ) : $oi++;
        ...
        ?>
          <div class="rh-serv__card" data-reveal data-reveal-delay="<?php echo esc_attr( ($oi-1) % 4 ); ?>">
            <div class="rh-serv__ic"><?php echo rh_icon( $oic, 26 ); ?></div>
            <h3 class="rh-serv__t"><?php echo esc_html( get_the_title( $svc ) ); ?></h3>
            <p class="rh-serv__d"><?php echo esc_html( wp_trim_words( $od, 18 ) ); ?></p>
            <a class="rh-serv__link" href="<?php echo esc_url( get_permalink( $svc ) ); ?>" style="margin-top:auto">Learn more <?php echo rh_icon('arrow',16); ?></a>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
  ...
  <section class="rh-section">
    <div class="rh-wrap">
      <div class="rh-head-center" data-reveal><span class="rh-eyebrow"><?php echo rh_icon('report',14); ?> FAQ</span><h2 class="rh-h2">Frequently asked questions</h2></div>
      <div class="rh-faq">
        <?php foreach ( $faqs as $qi => $faq ) : ?>
          <div class="rh-acc<?php echo $qi === 0 ? ' is-open' : ''; ?>" data-reveal>
            ...
            <div class="rh-acc__a"<?php echo $qi === 0 ? ' style="max-height:400px"' : ''; ?>><p><?php echo wp_kses_post( $faq['answer'] ?? '' ); ?></p></div>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
  ...
  <section class="rh-section rh-final">
    ...
    <div class="rh-wrap" style="position:relative;z-index:2">
      <div data-reveal>
        ...
        <h2 class="rh-final__title">Ready to grow your business with <?php echo esc_html( $title_short ); ?>?</h2>
        <p class="rh-lead" style="margin:0 auto;color:#AFC0E8">Book a free strategy call. We&rsquo;ll audit your current performance and show you exactly where the biggest growth opportunities are.</p>
        <div class="rh-final__cta">
          <a class="rh-btn rh-btn--primary" href="<?php echo esc_url( home_url('/contact/') ); ?>"><?php echo rh_icon('rocket',18); ?> Get Free SEO Audit</a>
